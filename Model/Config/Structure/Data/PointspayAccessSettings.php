...
class PointspayAccessSettings extends AbstractChain implements StructureDataUpdaterInterface
{
    ...
    protected function restructurePayments(array $config)
    {
        ...
        foreach ($this->paymentsReader->getAvailablePointspayMethods() as $key => $method) {
            ...
            $methodCodeForAccess = str_replace('_required_settings', '', $methodCode) . '_access_settings';
            if (!$this->arrayManager->exists($startPath . '/children/' . PointspayGeneralPaymentInterface::POINTSPAY_GENERAL_SETTINGS . '/children/' . $methodCodeForAccess, $sectionData)) {
                ...
                array_walk_recursive(
                    ...
                    function (&$value, $key) use ($methodCode) {
                        if (is_string($value)) {
                            $newGroupName = str_replace('pointspay', $methodCode, PointspayGeneralPaymentInterface::POINTSPAY_ACCESS_SETTINGS);
                            $value = str_replace(PointspayGeneralPaymentInterface::POINTSPAY_ACCESS_SETTINGS, $newGroupName, $value);
                ...
                $fieldSetToCopy = $this->processFieldSet($fieldSetToCopy, $methodCode, $methodTitle);
                ...
                $sectionData = $this->arrayManager->set($startPath . '/children/' . $newFieldSetName . '/children/' . $methodCode . '_access_settings', $sectionData, $valueToClone);
                foreach ($payPalFix as $fixCountry) {
                    ...
                    $sectionData = $this->repeatForEachPayPalCountry($modifiedStartPath, $newFieldSetName, $sectionData, $fieldSetToCopy, $valueToClone, $methodCode . '_access_settings', $payPalFixWithOriginal, $fixCountry);
                }
            }
        }
        $config = $sectionData;
    ...
    protected function modifyChild($childrenItemsToIterate, $methodCode)
    {
        foreach ($childrenItemsToIterate as $keyChild => $valueChild) {
            if (is_string($valueChild)) {
                $valueChild = str_replace(PointspayGeneralPaymentInterface::POINTSPAY_ACCESS_SETTINGS, $methodCode, $valueChild);
                $newFieldSetName = str_replace('pointspay', $methodCode, PointspayGeneralPaymentInterface::POINTSPAY_GENERAL_SETTINGS);
